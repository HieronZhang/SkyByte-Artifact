...
fig, ax0 = plt.subplots(1, 1, figsize=(15, 2.5))
...
for j, setting in enumerate(settings):
  if inc_base:
    ...
    base = 0
  ax0.bar(x_tick_array[:, j], (data_array[:, j]*1000), color=color_arr[j], width=bar_width, edgecolor=hatch_color, hatch=hatch_arr[j], label=setting, zorder=3)
# for j, setting in enumerate(settings):
  ax0.bar(x_tick_array[:, j], (data_array[:, j]*1000), color="none", width=bar_width, edgecolor="white", linewidth=0.8, zorder=3)
...
if inc_base:
  # ax0.set_yscale('log')
  ax0.set_yticks(np.arange(0, 2000, 500))
  # ax0.set_yticklabels(["1", "3", "9", "27"])
  ax0.set_ylim([0, 2000])
else:
...
if inc_base:
  fig.savefig(f"output/BW_util_motivation.png", bbox_inches=extent)
  fig.savefig(f"output/BW_util_motivation.pdf", bbox_inches=extent)
else:
