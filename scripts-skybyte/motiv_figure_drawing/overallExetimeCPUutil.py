...
if inc_base:
  data_file = f"exetime_cpu_util.txt"
else:
...
percent_array = np.zeros((len(sections) - 1))
bottom = np.zeros((len(sections) - 1))
# settings = settings[:-1]
...
for section_idx, section in enumerate(sections[1:]):
  ...
  percent_array[section_idx] = np.array(int(lines[2].strip()))
...
for j, setting in enumerate(settings):
  if inc_base:
    ...
    base = 0
  if j == len(settings)-1:
    ax0.bar(x_tick_array[:, j], data_array[:, 0] / (data_array[:, j]*(percent_array)/100), color=color_arr[j], width=bar_width, edgecolor=hatch_color, hatch=hatch_arr[j], label="SkyByte-Full", zorder=3)
    bottom = data_array[:, j]*percent_array/100
  else:
    ax0.bar(x_tick_array[:, j], data_array[:, 0] / data_array[:, j] , color=color_arr[j], width=bar_width, edgecolor=hatch_color, hatch=hatch_arr[j], label=setting, zorder=3)
...
# ax0.bar(x_tick_array[:, len(settings)-1], data_array[:, len(settings)-1]-bottom , color=color_arr[len(settings)], width=bar_width, edgecolor=hatch_color, hatch=hatch_arr[len(settings)], label="SkyByte-Full-Idle", zorder=3, bottom=bottom-2)
...
if inc_base:
  legend = ax0.legend(handles, labels, frameon=False, loc="upper center", bbox_to_anchor=(0.7, 1.05), ncol=3, columnspacing=2.5)
else:
...
if inc_base:
  fig.savefig(f"output/Exetime_cpu_util.png", bbox_inches=extent)
  fig.savefig(f"output/Exetime_cpu_util.pdf", bbox_inches=extent)
else:
