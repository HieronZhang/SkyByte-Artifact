...
if inc_base:
  data_file = f"cachesize_scan_avg_lat.txt"
else:
...
if inc_base:
  ax0.set_yscale('log')
  tick_values = [128, 512, 2048, 8192, 32768, 131072, 524288]
  ax0.set_yticks(tick_values)
  ax0.set_yticklabels(tick_values)
  ax0.set_ylim([46, 2000000])
else:
  ax0.set_yticks(np.arange(0, 1.8, 0.25))
  ax0.set_ylim([0, 2.1])
ax0.set_ylabel("Average Memory\n Access Latency (ns)", fontsize=20)
...
if inc_base:
  legend = ax0.legend(handles, labels, frameon=False, loc="upper center", bbox_to_anchor=(0.5, 1.05), ncol=len(labels), columnspacing=1)
else:
  legend = ax0.legend(handles, labels, frameon=False, loc="upper center", bbox_to_anchor=(0.5, 1.05), ncol=len(labels), columnspacing=1)
...
if inc_base:
  fig.savefig(f"output/cachesize_scan.png", bbox_inches=extent)
  fig.savefig(f"output/cachesize_scan.pdf", bbox_inches=extent)
else:
