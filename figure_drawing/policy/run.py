...
ax0.set_xticklabels(["" for _ in x_major_ticks], fontsize=24)
ax0.xaxis.set_minor_locator(FixedLocator(x_minor_ticks))
for x_subtick, x_subtick_label in zip(x_subticks, x_subtick_labels):
  if x_subtick_full:
    ax0.text(x_subtick + bar_width / 2, ymin - (ymax - ymin) * 0.04, x_subtick_label, ha='right', va='top', fontsize=20, rotation=45)
  else:
...
for x_tick, x_tick_label in zip(np.array([horiz_major_tick * x_tick_idx for x_tick_idx in range(len(workloads))]), workloads):
  ax0.text(x_tick, ymin - (ymax - ymin) * (0.15 if not x_subtick_full else 0.38), x_tick_label, ha='center', va='top', fontsize=22)
...
legend = ax0.legend(handles, labels, loc="upper center", bbox_to_anchor=(0.5, 1.04), ncol=3, frameon=False, columnspacing=0.9, fontsize=18)
